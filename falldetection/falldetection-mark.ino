...
void setup() {
  ...
  // Set 'Fall time setting' to 5 seconds
  byte sensor_cmd1[] = {0x53, 0x59, 0x83, 0x0C, 0x00, 0x04, 0x05, 0x1DB, 0x54, 0x43};
  mySerial.write(sensor_cmd1, sizeof(sensor_cmd1));

  // Set 'Residence time setting' to 60 seconds
  byte sensor_cmd2[] = {0x53, 0x59, 0x83, 0x0A, 0x00, 0x04, 0x60, 0x234, 0x54, 0x43};
  mySerial.write(sensor_cmd2, sizeof(sensor_cmd2));

  // Set 'Fall sensitivity setting' to 3 units
  byte sensor_cmd3[] = {0x53, 0x59, 0x83, 0x0D, 0x00, 0x04, 0x03, 0x1DA, 0x54, 0x43};
  mySerial.write(sensor_cmd3, sizeof(sensor_cmd3));

  // Set 'Height accumulation time setting' to 5 seconds
  byte sensor_cmd4[] = {0x53, 0x59, 0x83, 0x0F, 0x00, 0x04, 0x05, 0x1DE, 0x54, 0x43};
  mySerial.write(sensor_cmd4, sizeof(sensor_cmd4));
...
void fall_detection() {
  if(sensor_report != 0x00){
    switch(sensor_report){
      case NOFALL:
        fall_msg = "The sensor detects this movement is not a fall.";
        break;
      case FALL:
        fall_msg = "The sensor detects a fall.";
        break;
      case NORESIDENT:
        fall_msg = "The sensors did not detect anyone staying in place.";
        break;
      case RESIDENCY:
        fall_msg = "The sensor detects someone staying in place.";
        break;
    }
  }
}
...
void loop() {
  ...
  if (millis() - lastFallDetectionTime >= 100) {
